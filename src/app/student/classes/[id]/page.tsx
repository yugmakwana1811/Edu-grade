...
export default async function StudentClass({
  params,
  searchParams,
}: {
  ...
  return (
    <div className="page">
      ...
      <PageHeader
        ...
        title={c.name}
        description={`${c.teacher.user.name} · ${c.description ?? "Your class learning space"}`}
      />
      ...
      >
          ...
          <div className="card card-pad">
            ...
            {c.resources.map((r) => (
              ...
              >
                ...
                <div>
                  <strong style={{ fontSize: ".84rem" }}>{r.title}</strong>
                  <div className="hint">{r.type}</div>
                </div>
                <Download size={16} />
              </a>
            ))}
        ...
        <aside className="card card-pad">
          <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
            ...
            <div className="eyebrow">Class notices</div>
          </div>
          {c.announcements.map((a) => (
            ...
            >
              ...
              <p
                className="hint"
                style={{ lineHeight: 1.5, margin: ".35rem 0" }}
              >
                {a.content}
              </p>
              <small className="hint">{formatDate(a.publishedAt)}</small>
            </article>
          ))}
          {c.quizzes.length > 0 && (
            <Link
